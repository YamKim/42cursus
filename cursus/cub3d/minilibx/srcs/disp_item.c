...
** sprite setting
*/
void	set_item(t_pair *itm_pair, t_player *player, t_lst *itm_lst, int itm_cnt)
{
	int		i;
	t_itm	itm;
	...
	while (i < g_itm_cnt)
	{
		itm = lst_get_idx(itm_lst, i)->itm;
		itm_pair[i].ord = i;
		itm_pair[i].dist = calc_dist(player->pos, itm.pos);
		++i;
	}	
}

t_vecd	get_coef_itm(t_player *p, t_pair *itm_pair, t_lst *itm_lst)
{
	t_vecd	ret;
	double	det;
	t_itm	itm;
...
void	sort_item_pair(t_pair *itm_pair)
{
	int		i;
	int		j;
	double	tmp_dist;
...
void	set_draw_item(t_disp disp, t_draw *draw, double y_dist)
{
	...
	g_move = (int)(ITEM_MOVE / y_dist);
	h = disp.h;
	w = disp.w;
	itm_h = (int)(fabs((double)h / y_dist));
	draw->lh = itm_h / ITEM_VDIV;
	draw->beg = (int)fmax((double)(h - draw->lh) / 2 + g_move, 0.0);
	draw->end = (int)fmin((double)(h + draw->lh) / 2 + g_move, h - 1);
...
void	draw_item_part(t_disp *disp, t_tex *tex, t_player *player, t_draw *draw)
{
	...
	while (draw->x < draw->xend)
	{
			...
			&& player->coef.y < g_perp_buf[draw->x])
		{
			...
			{
				tmp = ((draw->y - g_move) * 2 - disp->h+ draw->lh) * 128;
...
int		draw_item(t_disp disp, t_player p, t_hit hit_point, double *perp_buf)
{
	...
	g_perp_buf = perp_buf;
	g_itm_cnt = disp.itm_cnt;
	if (!(itm_pair = (t_pair *)malloc(sizeof(t_pair) * g_itm_cnt)))
		return (ERR_MALLOC);
	set_item(itm_pair, &p, disp.itm_lst);
	sort_item_pair(itm_pair);
	i = -1;
	while (++i < disp.itm_cnt)
	{
		p.coef = get_coef_itm(&p, &(itm_pair[i]), disp.itm_lst);
		draw.xctr = (int)((double)disp.w/ 2 * (1 + p.coef.x / p.coef.y));
		set_draw_item(disp, &draw, p.coef.y); 
		tex = disp.tex[lst_get_idx(disp.itm_lst, itm_pair[i].ord)->itm.tex_nbr];
		draw_item_part(&disp, &tex, &p, &draw);
	}
	free(itm_pair);
	return (1);
}
